...
// =====================================================
// Helper: reuse K-LUT DSD (|My|<=K, |Mx|<=K-1) to build a small chain.
    ...
    const std::unordered_map<int, int>* placeholder_nodes)
{
    ...
    // Base case: direct K-LUT
    if (order.size() <= static_cast<size_t>(LUT_PAIR_FANIN_LIMIT))
    {
        auto children =
...
// 2) If either side has <= K vars, stop there; otherwise reuse K-LUT flow
...
inline bool run_66lut_else_dec_and_build_dag(const TT& root_tt)
{
    ...
    // Build MY (stop immediately if it already fits in a K-LUT)
    ...
    int root_node = -1;
    if ((int)order_mx.size() <= LUT_PAIR_FANIN_LIMIT)
    {
