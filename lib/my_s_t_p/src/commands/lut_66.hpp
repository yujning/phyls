...
public:
    explicit lut_66_command(const environment::ptr& env)
         : command(env, "XX-LUT decomposition (disjoint-first)")
    {
        ...
        add_option("-x", lut_k,
                   "target LUT size for pair decomposition (supported: 4 or 6)");
...
protected:
    ...
    void execute() override
    {
        ...
        if (lut_k != 4 && lut_k != 6)
        {
            std::cout << "❌ -x only supports 4 or 6.\n";
            return;
        }

        LUT_PAIR_FANIN_LIMIT = lut_k;
        ...
        if (shrunk_vars <= static_cast<unsigned>(LUT_PAIR_FANIN_LIMIT))
        {
            std::cout << "🔀 Mode: single " << LUT_PAIR_FANIN_LIMIT << "-LUT (no decomposition needed)\n";
        ...
        if (only_bidec && use_else_dec)
        {
         ...
         if (only_dsd || (!only_bidec && !use_else_dec))
        {
             std::cout << "🔀 Mode: XX-LUT Strong DSD (disjoint detection)\n";
            ...
            std::cout << "⚠️ DSD failed, falling back to XX-LUT bi-decomposition...\n";
        }

        std::cout << "🔀 Mode: XX-LUT Bi-Decomposition\n";
...
struct lut_66_command_init
{
    lut_66_command_init()
    {
        alice_globals::get().command_names.emplace_back("xxl", "STP");
    }
};
