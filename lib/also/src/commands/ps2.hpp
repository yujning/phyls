    ...
    public:
        ...
        : command( env, "Show statistics in the stored network." )
      {
        ...
        add_flag( "--mig,-m", "Display stats for stored MIG" );
         add_flag( "--klut,-t", "Display stats for stored KLUT" );
      ...
      void dump_klut_stats()
      {
        if ( store<mockturtle::klut_network>().empty() )
        {
          env->err() << "[e] KLUT network not stored\n";
          return;
        }

        auto klut = store<mockturtle::klut_network>().current();
        const auto is_buffer_node = [&]( auto const& n ) {
          if ( klut.is_constant( n ) || klut.is_pi( n ) )
          {
            return false;
          }

          const auto func = klut.node_function( n );
          if ( func.num_vars() != 1u )
          {
            return false;
          }

          /* 1-input BUF truth table is 0x2 (10) */
          return !kitty::get_bit( func, 0u ) && kitty::get_bit( func, 1u );
        };

        uint32_t gate_count_without_buf = 0;
        klut.foreach_node( [&]( auto const& n ) {
          if ( klut.is_constant( n ) || klut.is_pi( n ) || is_buffer_node( n ) )
          {
            return;
          }
          ++gate_count_without_buf;
        } );

        std::unordered_map<mockturtle::klut_network::node, uint32_t> level_cache;
        std::function<uint32_t( mockturtle::klut_network::signal const& )> level_of_signal;
        level_of_signal = [&]( mockturtle::klut_network::signal const& s ) -> uint32_t {
          const auto n = klut.get_node( s );
          if ( klut.is_constant( n ) || klut.is_pi( n ) )
          {
            return 0u;
          }

          const auto it = level_cache.find( n );
          ...
          if ( is_buffer_node( n ) )
          {
            auto fanin = klut.get_constant( false );
            klut.foreach_fanin( n, [&]( auto const& f, auto ) {
              fanin = f;
            } );
            const auto l = level_of_signal( fanin );
            level_cache.emplace( n, l );
            return l;
          }

          uint32_t max_fanin_level = 0u;
          klut.foreach_fanin( n, [&]( auto const& f, auto ) {
            max_fanin_level = std::max( max_fanin_level, level_of_signal( f ) );
          } );
          const auto l = max_fanin_level + 1u;
          level_cache.emplace( n, l );
          return l;
        };

        uint32_t depth_without_buf = 0;
        klut.foreach_po( [&]( auto const& f ) {
          depth_without_buf = std::max( depth_without_buf, level_of_signal( f ) );
        } );

        env->out() << "KLUT   i/o = " << klut.num_pis() << "/" << klut.num_pos()
                   << "   gates = " << gate_count_without_buf
                   << "   level = " << depth_without_buf << "\n";
      }
    ...
    protected:
      void execute()
      {
       ...
       else if ( is_set( "klut" ) )
        {
          ...
          if ( is_set( "criti" ) )
          {
            env->out() << "[w] --criti is not supported for KLUT in ps2\n";
          }
          if ( is_set( "fanout" ) )
          {
            fanout_histogram<mockturtle::klut_network>( "KLUT" );
          }
          if ( is_set( "level" ) )
          {
            level_size<mockturtle::klut_network>( "KLUT" );
          }
          if ( is_set( "skip" ) )
          {
            skip_histogram<mockturtle::klut_network>( "KLUT" );
          }
          return;
        }
        else if ( is_set( "aig" ) )
