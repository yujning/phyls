...
public:
      ...
      : command( env, "tech map and logic optimization using mixed structural choice" )
  {
    ...
    add_flag( "--stp_dsd, -d", "run STP strong DSD (+else) on LUT result" );
  ...
  void execute()
  {
    ...
    if ( is_set( "aig" ) && !store<aig_network>().empty() )
    {
      ...
      else if ( is_set( "LUT" ) )
      {
        ...
        maybe_apply_stp_dsd_lut_resynthesis( klut );
...
private:
 choice_lut_map_params make_lut_choice_params() const
  {
    choice_lut_map_params lut_ps;
    lut_ps.cut_enumeration_ps.cut_size = cut_size;
    lut_ps.cut_enumeration_ps.cut_limit = cut_limit;
  ...
  void remap_lut_with_mch( klut_network& klut ) const
  {
    aig_network aig_after_stp = convert_klut_to_graph<aig_network>( klut );
    auto const lut_ps = make_lut_choice_params();
    ...
    case 0u:
    {
      xmg_choice_compute_params xmg_ps;
      xmg_ps.ratio = ratio;
      xmg_network xmg = also::xmg_from_aig( aig_after_stp );
      xmg_network xwc = run_xmg_choice_compute( xmg, xmg_ps );
      mapping_view<xmg_network, true> remapped{ xwc };
      also::choice_lut_map_inplace<decltype( remapped ), true>( remapped, lut_ps );
      klut = *choice_to_luts<klut_network>( remapped );
      break;
    }
    case 1u:
    {
      xag_choice_compute_params xag_ps;
      xag_ps.ratio = ratio;
      xag_network xag = also::xag_from_aig( aig_after_stp );
      xag_network xwc = run_xag_choice_compute( xag, xag_ps );
      mapping_view<xag_network, true> remapped{ xwc };
      also::choice_lut_map_inplace<decltype( remapped ), true>( remapped, lut_ps );
      klut = *choice_to_luts<klut_network>( remapped );
      break;
    }
    case 2u:
    {
      mig_choice_compute_params mig_ps;
      mig_ps.ratio = ratio;
      mig_network mig = also::mig_from_aig( aig_after_stp );
      mig_network mwc = run_mig_choice_compute( mig, mig_ps );
      mapping_view<mig_network, true> remapped{ mwc };
      also::choice_lut_map_inplace<decltype( remapped ), true>( remapped, lut_ps );
      klut = *choice_to_luts<klut_network>( remapped );
      break;
    }
    case 3u:
    {
      ...
      xmg_ps.ratio = ratio;
      mig_network mig = also::mig_from_aig( aig_after_stp );
      xmg_network xmg = also::xmg_from_mig( mig );
      xmg_network xwc = run_xmg_choice_compute( xmg, xmg_ps );
      mapping_view<xmg_network, true> remapped{ xwc };
      also::choice_lut_map_inplace<decltype( remapped ), true>( remapped, lut_ps );
      klut = *choice_to_luts<klut_network>( remapped );
    ...
    default:
    {
      mapping_view<aig_network, true> remapped{ aig_after_stp };
      lut_map_params remap_ps;
      remap_ps.cut_enumeration_ps.cut_size = cut_size;
      remap_ps.cut_enumeration_ps.cut_limit = cut_limit;
      lut_map<decltype( remapped ), true>( remapped, remap_ps );
      klut = *collapse_mapped_network<klut_network>( remapped );
      break;
    }
    }
  }
  void maybe_apply_stp_dsd_lut_resynthesis( klut_network& klut ) const
  {
    if ( !is_set( "stp_dsd" ) )
    {
      return;
    ...
    also::stp_dsd_lut_resynthesis<klut_network> stp_dsd_resyn;
    klut = node_resynthesis<klut_network>( klut, stp_dsd_resyn );

    /* remap after STP-DSD using MCH so decomposition happens before final mixed-choice LUT mapping */
    remap_lut_with_mch( klut );
  }
  ...
  aig_network run_aig_choice_compute( const aig_network& aig, const aig_choice_compute_params& params ) const
  {
    ...
    dsd_resynthesis<aig_network, decltype( sop_resyn )> dsd_resyn( sop_resyn );
     // also::stp_dsd_resynthesis<aig_network, decltype( sop_resyn )> stp_dsd_resyn( sop_resyn );
   ...
   return aig_choice_compute( choice_aig, npn_resyn, dsd_resyn, exact_lib,
    ...
    // return aig_choice_compute( choice_aig, npn_resyn, stp_dsd_resyn, exact_lib,
    //                            sop_resyn, stp_dsd_resyn, params );                               
  }
  ...
  xmg_network run_xmg_choice_compute( const xmg_network& xmg, const xmg_choice_compute_params& params ) const
  {
    ...
    dsd_resynthesis<xmg_network, decltype( sop_resyn )> dsd_resyn( sop_resyn );
     //also::stp_dsd_resynthesis<xmg_network, decltype( sop_resyn )> stp_dsd_resyn( sop_resyn );
  ...
  mig_network run_mig_choice_compute( const mig_network& mig, const mig_choice_compute_params& params ) const
  {
    ...
    dsd_resynthesis<mig_network, decltype( sop_resyn )> dsd_resyn( sop_resyn );
      // also::stp_dsd_resynthesis<mig_network, decltype( sop_resyn )> stp_dsd_resyn( sop_resyn );
  ...
  xag_network run_xag_choice_compute( const xag_network& xag, const xag_choice_compute_params& params ) const
  {
    ...
    dsd_resynthesis<xag_network, decltype( sop_resyn )> dsd_resyn( sop_resyn );
      //also::stp_dsd_resynthesis<xag_network, decltype( sop_resyn )> stp_dsd_resyn( sop_resyn );
   ...
   return xag_choice_compute( choice_xag, npn_resyn, dsd_resyn, exact_lib,
                               sop_resyn, dsd_resyn, params );
//  return xag_choice_compute( choice_xag, npn_resyn, stp_dsd_resyn, exact_lib,
//                                sop_resyn, stp_dsd_resyn, params );                             
  }
};
