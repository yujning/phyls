...
// 工具
// =====================================================
inline uint64_t strong_pow2(int k) { return 1ull << k; }
...
// =====================================================
inline std::string strong_reorder_tt_by_var_order(
    const std::string& mf,
    int n,
...
strong_enumerate_variable_partitions(const std::vector<int>& vars_msb2lsb,
                                     int x, int y, int z)
{
    std::vector<
...
// =====================================================
inline uint64_t strong_mf_index(uint64_t a, uint64_t b, uint64_t c, int y, int z)
{
    return (a << (y + z)) | (b << z) | c;
}
// =====================================================
...
// =====================================================
inline void strong_print_reordered_tt(
    const std::string& mf,
    int n,
...
// 打印 Mr / MZ
// =====================================================
inline void strong_print_MZ_delta(int x, int y)
{
    uint64_t Ix = strong_pow2(x);
    uint64_t t  = strong_pow2(y);
...
// Step 1：MF′ + MZ → MXY
// =====================================================
inline std::string strong_compute_MXYX_from_MF(
    const std::string& MF,
    int x, int y, int z)
{
    uint64_t HN = strong_pow2(x);
    uint64_t MN = strong_pow2(y);
    uint64_t LN = strong_pow2(z);

    uint64_t MXY_cols = strong_pow2(x + 2 * y + z);
    ...
    for (uint64_t b = 0; b < MN; b++)
    {
        ...
        {
            uint64_t mf_col  = strong_mf_index(a, b, c, y, z);
...
// =====================================================
inline bool strong_solve_MX_MY_from_MXY(
    ...
    std::string& MY)
{
    uint64_t MYN = strong_pow2(x + y);
    uint64_t block_size = strong_pow2(y + z);
...
// children 构造（含占位符）
// =====================================================
inline std::vector<int> strong_make_children_with_placeholder(
    const std::vector<int>& order_msb2lsb,
...
// ★ 入口函数（最终）
// =====================================================
inline bool run_strong_bi_dec_general_and_build_dag(const TT& root_tt)
{
    ...
    for (auto [x, y, z] : strong_enumerate_xyz(n))
    {
        auto parts = strong_enumerate_variable_partitions(original_order, x, y, z);
        for (const auto& [A, B, C] : parts)
        {
            ...
            std::string MFp =
                strong_reorder_tt_by_var_order(root_tt.f01, n, new_order, original_order);

            strong_print_reordered_tt(MFp, n, new_order);
            ...
            strong_print_MZ_delta(x, y);

            std::string MXY =
                strong_compute_MXYX_from_MF(MFp, x, y, z);
            std::cout << "🟨 MXY = " << MXY << "\n";

            std::string MX, MX_with_x, MY;
            if (!strong_solve_MX_MY_from_MXY(MXY, x, y, z, MX, MX_with_x, MY))
            {
            ...
            auto children_mx = strong_make_children_with_placeholder(
...
inline bool run_strong_bi_dec(const std::string& binary01)
{
    int n = static_cast<int>(std::log2(binary01.size()));
    if ((size_t(1) << n) != binary01.size())
        return false;
    ...
    std::cout << "======= Strong Bi-Decomposition start =======\n";
    ...
    return run_strong_bi_dec_general_and_build_dag(root_shrunk);
}